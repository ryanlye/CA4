...
public class MemberWS {
    ...
    /**
     * Retrieves representation of an instance of services.MemberWS
    ...
    public Response getMember(@QueryParam("email") String Email)throws ClassNotFoundException, SQLException 
 {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Member member = new Member(id, phone, address, city, securityQuestion, securityAnswer, 
                        age, income, name, email, loyaltyPoints, cumulativeSpending);
                mem.add(member);

            }
            System.out.println(mem.size()); //for debugging purpose
            GenericEntity<ArrayList<Member>> entity = new GenericEntity<ArrayList<Member>>(mem) {};
            return Response
                    .status(200)
                    .entity(entity)
                    .build();
